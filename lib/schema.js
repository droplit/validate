...
Schema.prototype.additionalProps = function (obj, prefix) {
  var errors = [];
  for (var key in obj) {
    var path = join(key, prefix);

    // skip properies inherited from a prototype
    if (!obj.hasOwnProperty(key)) continue;

console.log(obj, key, this.props);
    if (!this.props[path]) {
      errors.push(error(path, "invalid property"));
      console.log('error found', path);
      continue;
    }

    if (typeOf(obj[key]) == 'object') {
      errors = errors.concat(this.additionalProps(obj[key], path));
    }
  }
  return errors;
};
...
Schema.prototype.validate = function (obj, opts) {
  var errors = [];
  opts = opts || this.opts;
  opts.allowAdditionalProps = opts.allowAdditionalProps === undefined || opts.allowAdditionalProps === true ? true : false;

  if (opts.typecast) this.typecast(obj);
  if (opts.strip !== false && opts.allowAdditionalProps === true) this.strip(obj);
  ...
  if (!opts.allowAdditionalProps) errors = errors.concat(this.additionalProps(obj));
